...
<body>
    ...
    <div class="admin-container">
        ...
        <?php if ($order_detail): ?>
            <a href="<?php echo url('/admin/orders.php'); ?>" class="btn-back">Back to Orders</a>
            <div class="order-detail-header">
                <div>
                    <h1>Order #<?php echo str_pad($order_detail['order_id'], 6, '0', STR_PAD_LEFT); ?></h1>
                    <p class="order-date-display"><?php echo date('F j, Y \a\t g:i A', strtotime($order_detail['order_date'])); ?></p>
                </div>
                <div style="display:flex; align-items:center; gap:12px;">
                    <?php $sc = $status_config[$order_detail['order_status']] ?? $status_config['pending']; ?>
                    <span class="order-status <?php echo $sc['class']; ?>"><?php echo $sc['label']; ?></span>
            ...
            <div class="order-detail-layout">
                <div class="order-detail-card">
                    <h2>Items (<?php echo count($order_items); ?>)</h2>
                ...
                <div class="order-info-sidebar">
                    <div class="order-detail-card"><h2>Customer</h2><div class="info-block"><span class="info-label">Name</span><p class="info-value"><?php echo htmlspecialchars($order_detail['full_name']); ?></p></div><div class="info-block"><span class="info-label">Email</span><p class="info-value"><?php echo htmlspecialchars($order_detail['email']); ?></p></div><div class="info-block"><span class="info-label">Phone</span><p class="info-value"><?php echo htmlspecialchars($order_detail['phone'] ?? 'N/A'); ?></p></div></div>
                    <div class="order-detail-card"><h2>Shipping</h2><div class="info-block"><span class="info-label">Address</span><p class="info-value"><?php echo nl2br(htmlspecialchars($order_detail['shipping_address'])); ?></p></div></div>
                    <div class="order-detail-card"><h2>Payment</h2><div class="info-block"><span class="info-label">Method</span><p class="info-value"><?php echo $payment_labels[$order_detail['payment_method']] ?? $order_detail['payment_method']; ?></p></div></div>
                    <?php if ($order_detail['notes']): ?><div class="order-detail-card"><h2>Notes</h2><p class="info-value"><?php echo htmlspecialchars($order_detail['notes']); ?></p></div><?php endif; ?>
                </div>
            </div>
        ...
        <?php else: ?>
            ...
            <?php if ($flash): ?>
                <div class="alert alert-<?php echo $flash['type']; ?>"><span><?php echo $flash['message']; ?></span></div>
            <?php endif; ?>
            ...
            <div class="admin-card">
                <div class="admin-card-header">
                    <h3><?php echo !empty($status_filter) ? ucfirst($status_filter) . ' Orders' : 'All Orders'; ?> (<?php echo count($orders); ?>)</h3>
                    <?php if (!empty($status_filter)): ?><a href="<?php echo url('/admin/orders.php'); ?>" class="card-link">Clear</a><?php endif; ?>
                </div>
                        ...
                        <?php if (!empty($orders)): ?>
                            <?php foreach ($orders as $order): ?>
                                ...
                                <tr>
                                    ...
                                    <td><span class="order-status <?php echo $sc['class']; ?>"><?php echo $sc['label']; ?></span></td>
                                    <td><?php echo date('M j, Y', strtotime($order['order_date'])); ?></td>
                                    <td>
                                        <div class="table-actions">
                                            <a href="<?php echo url('/admin/orders.php?view=' . $order['order_id']); ?>" class="action-btn action-edit" title="View">View</a>
                                            <form method="POST" action="<?php echo url('/admin/orders.php'); ?>" class="status-form">
                                                <input type="hidden" name="action" value="update_status">
